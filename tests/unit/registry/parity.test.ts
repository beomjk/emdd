...
describe('FR-018: CLI/MCP Parity', () => {
  ...
  it('real registry: all 26 commands exposed to both CLI and MCP', async () => {
    const registry = createDefaultRegistry();
    const allCommands = registry.getAll();

    // 27 commands total; read-nodes is MCP-only (cli: false)
    expect(allCommands.length).toBe(27);
    expect(allCommands.filter(c => c.cli !== false).length).toBe(26);
    expect(allCommands.every(c => c.mcp !== false)).toBe(true);

    // Verify CLI adapter registers CLI-visible commands only
    const program = new Command().name('emdd').exitOverride();
