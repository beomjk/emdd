...
```
PROPOSED → TESTING       : 연결된 Experiment가 RUNNING 또는 COMPLETED
...
│       └── emdd-close/SKILL.md  # /emdd-close 세션 종료 스킬
│
├── graph/                     # ★ EMDD 지식 그래프
│   ├── _index.md              # 자동 생성 인덱스
...
#   create-edge(source, target, relation, strength?, severity?, completeness?, dependencyType?, impact?, force?) → 에지 추가
...
#   update-node(nodeId, set, transitionPolicy?) → 프론트매터 필드 업데이트
#   mark-done(episodeId, item, marker?) → 에피소드 체크리스트 항목 표시
...
3. **CLI-Slash 통합**: MCP 프롬프트(`context-loading`, `episode-creation`, `consolidation`, `health-review`)가 CLI 커맨드를 직접 호출하도록 템플릿 재작성. Claude Code에서는 스킬로 노출됨: `/emdd-open` → `context-loading` 호출, `/emdd-close` → `episode-creation` → `consolidation` → `health-review` 순차 호출. 새 CLI 커맨드 4개 추가:
