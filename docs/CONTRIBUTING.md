...
# Build (schema codegen → TypeScript compile → copy rules → build web frontend)
npm run build
...
npm run gen:schema     # Regenerate types.generated.ts from graph-schema.yaml
...
src/graph/types.ts       ← interfaces + re-exports from types.generated.ts
src/graph/types.generated.ts ← auto-generated (DO NOT EDIT)
        ↑
graph-schema.yaml        ← SINGLE SOURCE OF TRUTH for all constants
```

**Important:** `types.generated.ts` is auto-generated by `npm run gen:schema`. Never edit it directly — modify `graph-schema.yaml` instead.

### Adding a New Node Type, Status, or Edge

1. Edit `graph-schema.yaml`
2. Run `npm run build` (regenerates types + spec tables)
